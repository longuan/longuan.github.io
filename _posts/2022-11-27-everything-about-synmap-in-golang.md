...
就像map无法应对并发场景的问题，sync.Map也不可能做到完美。但是针对某些场景做到更优，还是非常有价值和可行的。那么需要sync.Map来解决的场景有哪些典型特征呢？

- **stable keys**：  key集合是稳定的。比如统计每个省份有多少人，省份名称作为key，人数作为value，key总共有34个。
- disjoint stores： 多个CPU core不会同时对同一个key写入，最理想的是write-once, read-many。否则还是会出现RWMutex.RLock那样的cache contention情况。
- concurrent loops：多core并发，且有大量写入。否则使用sync.RWMutex+map也是个不错  的选择。

上面三个是sync.Map作者认为的最适用sync.Map的场景的几个特征。

比如标准库中的一个场景：Lazy construction，也是sync.Map的original motivating example。
-  reflect type、json.Encoder等等，具有stable keys特征（类型有限），虽然会有一个warm-up的过程

这里提出一个问题：sync.Map作者为什么认为具有上面三个特征的场景是最适合sync.Map？sync.Map分别做了哪些优化？
...
- 那么就加一个read-only的缓存，作为fast path。read-only可以消除cache contention。
...
下面也说一下sync.Map的**缺点**：
1. 占用内存比map略有上升，但不多
2. 指针嵌套有点深，但在适用场景下影响不大
3. 需要额外使用type-assertion
4. 换了一套API，并且不全（比如没有len方法）
